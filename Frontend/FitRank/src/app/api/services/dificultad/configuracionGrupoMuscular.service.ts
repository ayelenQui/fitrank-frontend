...
import { AgregarConfiguracionGrupoMuscularDTO, ConfiguracionGrupoMuscularDTO } from '../configuracionGrupoMuscular/interfaces/configuracionGrupoMuscular.interface';
...
export class ConfiguracionGrupoMuscularService {
  ...
  obtenerActual(): Observable<ConfiguracionGrupoMuscularDTO | null> {
    return new Observable(observer => {
      this.obtenerTodas().subscribe({
        next: configs => {
          if (configs && configs.length > 0) observer.next(configs[0]);
  ...
  actualizarConfiguracion(id: number, config: AgregarConfiguracionGrupoMuscularDTO) {
    return this.http.put<ConfiguracionGrupoMuscularDTO>(`${this.apiUrl}/${id}`, config);
  }
